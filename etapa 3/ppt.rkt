...
(define (apply-transf-helper Ts Fs tuple res)
  (if (null? Ts)
      res
      (cond
        [(= (car Ts) 1) (apply-transf-helper (cdr Ts) Fs tuple ((car Fs) res))]
        [(= (car Ts) 2) (apply-transf-helper (cdr Ts) Fs tuple ((cadr Fs) res))]
        [(= (car Ts) 3) (apply-transf-helper (cdr Ts) Fs tuple ((caddr Fs) res))])))

(define (apply-matrix-transformations Ts Fs tuple)
  (apply-transf-helper Ts Fs tuple tuple))

(define (get-nth-tuple Fs tuple)
  (λ (n) (apply-matrix-transformations (get-transformations n) Fs tuple)))
...
(define (get-nth-ppt-from-matrix-transformations n)
  ((get-nth-tuple (list FT1 FT2 FT3) '(3 4 5)) n))
...
(define (get-nth-quadruple n)
  ((get-nth-tuple (list FQ1 FQ2 FQ3) '(1 1 2 3)) n))
...
(define (get-nth-ppt-from-GH-quadruples n)
  (define cvartet (get-nth-quadruple n))
  (define a (* (car cvartet) (car (reverse cvartet))))
  (define b (* 2 (cadr cvartet) (cadr (reverse cvartet))))
  (define c (+ (expt (cadr cvartet) 2) (expt (cadr (reverse cvartet)) 2)))
  (list a b c))