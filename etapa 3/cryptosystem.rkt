...
(define (key n)
  (let* ([tuple (get-nth-quadruple n)]
        ...
        [C2 (c2 e f)]
        [C3 (c3 e f)])
    (map (λ (x) (modulo x 27)) (list A1 B1 C1 A2 B2 C2 A3 B3 C3))))
...
(define (message->codes message)
  (map (λ (x) (if (equal? x -64) 0 x))
       (map (λ (x) (- (char->integer x) 96)) (string->list message))))
...
(define (codes->message codes)
  (list->string (map (λ (x) (integer->char (+ (if (equal? x 0) -64 x) 96))) codes)))
...
(define (extend-key key size)
  (letrec ([extend-key-helper (λ (key size)
                                (cond
                                  [(equal? (length key) size) key]
                                  [(< (length key) size) (extend-key-helper (append key key) size)]
                                  [(> (length key) size) (extend-key-helper (reverse (cdr (reverse key))) size)]))])
...
(define (codes message key func)
  (map (λ (message key) (modulo (func message key) 27)) message (extend-key key (length message))))
...
(define (strings message key func)
  (codes->message (map (λ (message key) (modulo (func message key) 27)) (message->codes message) (extend-key key (length (message->codes message))))))
